...
export function cmdShow(args: string[]) {
  ...
  if (feature) {
    showFeature(feature, decisionsOn)
    return
  }
...
function showFeature(feature: ReturnType<typeof loadStore>['features'][0], decisionsOn = true) {
  ...
  if (decisionsOn && feature.decisions?.length) {
    console.log()
...
function showIssue(issue: ReturnType<typeof loadStore>['issues'][0], decisionsOn = true) {
